...
ScopedValue<Integer> YEAR = ScopedValue.newInstance();
...
List<YearStat> calculate(int from, int to) {
    try (var taskScope = new StructuredTaskScope.ShutdownOnFailure()) {
        List<StructuredTaskScope.Subtask<YearStat>> subtasks = IntStream.rangeClosed(from, to)
            .mapToObj(i -> taskScope.fork(() -> ScopedValue.where(YEAR, i)
                    .call(() -> collectYearStat())))
            .toList();
...
DateStat collectStat(LocalDate date) {
